...
const sandbox = new LocalSandbox({
  workingDirectory: './workspace',
  isolation: 'seatbelt', // Use 'bwrap' on Linux, 'wsl2' on Windows
  nativeSandbox: {
    allowNetwork: false,
...
Enable native isolation to restrict filesystem and network access at the operating-system level. On macOS, native isolation uses Seatbelt (`sandbox-exec`). On Linux, it uses Bubblewrap (`bwrap`). On Windows, it runs commands inside a WSL2 distro — see [WSL2 setup](/reference/workspace/local-sandbox#windows-wsl2-setup) for the one-time distro configuration this requires. Use `LocalSandbox.detectIsolation()` to check which backend is available on the current operating system.
